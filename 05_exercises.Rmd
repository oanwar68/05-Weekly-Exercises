...
#I prefer this to the static map because I feel as if we get more information with the same visual, and it's easier to track when animated
```
...
covid_anim <- covid19 %>% 
  ...
  geom_text(aes(x=cases,
                y= new_cases_7day,
                group = state,
                label=state),check_overlap = TRUE)+
...
#New York took off in terms of total cases as well as new cases from the beginning and stayed consistently high. California, Texas and Florida also had high number of total cases and new cases as well. 
```
...
This animation shows the rate that states had of per capita cases. Per capita, the midwest seemed to be the worst in terms of infectious rates. However, the rate of infections look bad across the board. 
